...
def import_directory(
    path: str,
    glucose_col: str = "Glucose Value (mg/dL)",
    time_col: str = "Timestamp (YYYY-MM-DDThh:mm:ss)",
    interval: int = 5,
) -> pd.DataFrame:
...
def interpolate_data(df: pd.DataFrame, max_gap: int = 45) -> pd.DataFrame:
    ...
    for i in range(len(df) - 1):
        ...
        # end of time gap needing interpolation
            ...
            if index_range["end"] - index_range["start"] <= pd.Timedelta(
                minutes=max_gap
            ):
